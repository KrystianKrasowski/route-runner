                          ...
                          linebot_motion_t * const handle)
{
    ...
    return linebot_motion_acquire(direction, correction, handle);
}
                          ...
                          linebot_motion_t * const handle)
{
    ...
    {
        result = linebot_motion_acquire(LINEBOT_MOTION_NONE, 0, handle);
    }
    else
    {
        int8_t correction = position_regulate(position);
        result = linebot_motion_acquire(LINEBOT_MOTION_FORWARD, correction, handle);
    }
...
{
    return linebot_motion_acquire(LINEBOT_MOTION_NONE, 0, handle);
}
